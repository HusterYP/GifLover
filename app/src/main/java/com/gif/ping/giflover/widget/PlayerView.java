...
public class PlayerView extends FrameLayout {
    ...
    public PlayerView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (attrs != null) {
            ...
            try {
                ...
                controllerAutoShow = a.getBoolean(R.styleable.PlayerView_auto_show,
                        controllerAutoShow);
                ...
                controllerHideDuringAds =
                        a.getBoolean(R.styleable.PlayerView_hide_during_ads,
                                controllerHideDuringAds);
            } finally {
                a.recycle();
            }
        ...
        @SuppressLint("WrongViewCast") PlayerControlView customController = findViewById(R.id.exo_controller);
    ...
    public static void switchTargetView(
        ...
        // We attach the new view before detaching the old one because this ordering allows the
        // player
        // to swap directly from one surface to another, without transitioning through a state
        // where no
     ...
     * wish to use this method directly, be sure to attach the player to the new view
     * <em>before</em>
    ...
    /**
     * Sets whether the playback controls can be shown. If set to {@code false} the playback
     * controls
     ...
     * player is reset. A player reset is defined to mean the player being re-prepared with
     * different
     ...
     * <p>
     * <p>If enabled, the currently displayed video frame or media artwork will be kept visible
     * until
     ...
     * <p>
     * <p>If disabled, the currently displayed video frame or media artwork will be hidden as
     * soon as
     * the player is reset. Note that the video frame is hidden by making {@code exo_shutter}
     * visible.
     * Hence the video frame will not be hidden if using a custom layout that omits this view.
     *
     * @param keepContentOnPlayerReset Whether the currently displayed video frame or media
     *                                 artwork is
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
        if (player != null && player.isPlayingAd()) {
            // Focus any overlay UI now, in case it's provided by a WebView whose contents may
            // update
            // dynamically. This is needed to make the "Skip ad" button focused on Android TV
            // when using
     ...
     * #getControllerShowTimeoutMs()}}. They are shown indefinitely when playback has not started
     * yet,
     ...
     *
     * @param controllerShowTimeoutMs The timeout in milliseconds. A non-positive value will
     *                                cause the
    ...
    /**
     * Sets whether the playback controls are automatically shown when playback starts, pauses,
     * ends,
     ...
     * @param extraPlayedAdGroups Whether each ad has been played, or {@code null} to show no
     *                            extra ad
     *                            markers.
     */
    ...
    /**
     * Gets the overlay {@link FrameLayout}, which can be populated with UI elements to show on
     * top of
     * the player.
     *
     * @return The overlay {@link FrameLayout}, or {@code null} if the layout has been customized
     * and
    ...
    private void maybeShowController(boolean isForced) {
        ...
        if (useController) {
            boolean wasShowingIndefinitely = controller.isVisible() && controller
                    .getShowTimeoutMs() <= 0;
    ...
    private void hideArtwork() {
        if (artworkView != null) {
            artworkView.setImageResource(android.R.color.transparent); // Clears any bitmap
            // reference.
        ...
        public void onVideoSizeChanged(
            ...
            if (surfaceView instanceof TextureView) {
                ...
                if (unappliedRotationDegrees == 90 || unappliedRotationDegrees == 270) {
                    // We will apply a rotation 90/270 degree to the output texture of the
                    // TextureView.
